...
#        Method 1         #
###########################
## Only possible if all data to be extracted
## have common charactersin their headers
...
tbl_df(x)
#View data in summarised spreadsheet
View(x)
...
tbl_df(x)
names(x)
#Selecting climate values for winter months only
##NOTE: cannot use "contains" with prec_1 or any pr701 because I do not want prec_10 or pr7010
...
x2 %>%  #Get some summary stats on each scenario (mean,  St. Dev.,  Coef. of Variation)
...
write.csv (x2, file = "winter_precip_formatted_boxplot.csv",row.names=FALSE)